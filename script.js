...
//   const loadingOverlay = document.getElementById("loading-overlay");
//   const imgContainer = document.querySelector(".image");
//   const images = document.querySelectorAll("#Profile"); // Updated selector
...
function displayFreeFriends() {
  ...
  const loadingOverlay = document.querySelector(".loading-overlay"); // Updated selector
  const imgContainer = document.querySelector(".image");
  const images = document.querySelectorAll(".profile-img"); // Updated class
  ...
  setTimeout(() => {
    for (const friend in timetableData) {
      const friendId = friend.toLowerCase();
      const img = document.getElementById(`${friendId}`); // Updated ID
      if (timetableData[friend][i][j] === "Free" && friendId !== selectedFriend.toLowerCase()) {
        img.style.filter = 'grayscale(0%)';
      } else {
        img.style.filter = 'grayscale(100%)';
